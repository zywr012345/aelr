...
export function getPosition (callback, start) {
  if (navigator.geolocation) {
    ...
    let temp = navigator.geolocation[func](function (position) {
      window.clearTimeout(tmid)
      let res = { code: 10000, message: '', result: { lng: position.coords.longitude, lat: position.coords.latitude } }
      callback(res)
    }, function (error) {
      window.clearTimeout(tmid)
      let message = ''
...
export function addMarker (info, map) { // 给地图添加标记
  let lng = info.lng
  let lat = info.lat
  let text = info.text
  ...
  marker.initialize = function (map) {
    ...
    div.innerHTML = '<b style="font-size:18px;position:absolute;top:0;left:0px;color:#fff;line-height:1.8;width:100%">' + text + '</b>'
...
export function addMarker2 (info, map) { // 给地图添加标记
  let lng = info.lng
  let lat = info.lat
...
export const getAddressByPoint =
    (location) =>
      requestRaw(
        env.SITE_URL + '/bmap/geocoder/v2/',
        'GET',
        {
          location: location,
          output: 'json',
...
export const getPointByAddress =
    (address) =>
      requestRaw(
        env.SITE_URL + '/bmap/geocoder/v2/',
        'GET',
...
export const getAddressByKeyword =
    (query, region) =>
      requestRaw(
        env.SITE_URL + '/bmap/place/v2/suggestion',
        'GET',
        {
          query: query,
          region: region,
...
export const getPointNearby =
    (location) =>
      requestRaw(
        env.SITE_URL + '/bmap/place/v2/search',
        'GET',
        {
