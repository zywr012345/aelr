    ...
    <div class="container">
        ...
        >
            ...
            <div class="address">
                ...
                <div v-if="order_info.code_list && order_info.code_list.length" class="vr_code">
                    <span v-for="(item ,index) in order_info.code_list" :key="index">
                        <span>兑换码{{item.vr_code}}<i @click="showQrcode(item.vr_code)" v-if="item.vr_state==0 && item.vr_indate>$moment().unix() && item.refund_lock==0" class="qrcode iconfont">&#xe64a;</i></span><br><span class="state">{{item.vr_code_desc}}</span>
                    </span>
                </div>
            </div>
...
export default {
  ...
  methods: {
    showQrcode (vr_code) {
      MessageBox({
        title: '兑换码' + vr_code,
        message: '<div class="common-alert-content"><img src="'+env.SITE_URL+'/home/qrcode?url='+encodeURIComponent(env.H5_HOST+'/seller/vrorder_list?vr_code='+vr_code)+'"><div class="alert-notice">请让卖家扫码或者输入兑换码进行核销</div></div>',
...
<style lang="scss" scoped>
    ...
    .address {
        ...
        span {
            color: #333;
            font-size:0.8rem;
            .iconfont {float:right;font-size:.8rem;line-height:1.5}
        }
        p {
