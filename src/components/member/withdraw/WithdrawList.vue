  ...
  <div class="container">
    ...
    <mt-popup v-model="addWithdrawVisible" position="right" class="common-popup-wrapper">
      ...
      <div class="common-popup-content">
        ...
        <mt-cell title="收款银行" v-if="bank_length > 0">
          <mt-radio
                  v-model="memberbank_id"
                  :options="memberbankOptions">
          </mt-radio>
        </mt-cell>
        <mt-button type="primary" @click='addWithdraw' class="ds-button-large"  v-if="bank_length > 0">提交</mt-button>
        <mt-button type="primary" @click='goMemberbacklist' class="ds-button-large"  v-else>请先添加收款账号</mt-button>
      </div>
    </mt-popup>
  </div>
</template>
