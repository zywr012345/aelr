...
<template>
	<div v-if="detailInfo">
		...
		<div class="ui-detail-info">
			...
			<div class="price">
			<span class="current-price">￥<slot v-if="detailInfo.bargain_info">{{ detailInfo.bargain_info.bargain_floorprice }}</slot><slot v-else-if="detailInfo.pintuan_type==1">{{ detailInfo.pintuan_price }}</slot><slot v-else-if="detailInfo.promotion_type">{{ detailInfo.promotion_price }}</slot><slot v-else>{{ detailInfo.goods_price }}<span class="current-price1">[{{priseName}}{{ pricePer}}%]</span></slot>
			</span>
...
//修改价格
	xgprise(){
		...
		if (!this.detailInfo.goods_price1){
			this.detailInfo.goods_price1=9	
		}
		this.pricePer=this.detailInfo.goods_price1*10
