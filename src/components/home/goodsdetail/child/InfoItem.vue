...
￥
<slot v-if="detailInfo.bargain_info">{{ detailInfo.bargain_info.bargain_floorprice }}</slot>
<slot v-else-if="detailInfo.pintuan_type==1">{{ detailInfo.pintuan_price }}</slot>
<slot v-else-if="detailInfo.promotion_type">{{ detailInfo.promotion_price }}</slot>
<slot v-else>{{ detailInfo.goods_price }}<span v-if="mclass>0" class="current-price1">[{{priseName}}{{ this.realprice}}]</span></slot>
			</span>
				<span class="old-price" v-if="detailInfo.goods_marketprice > 0">￥{{ detailInfo.goods_marketprice }}</span>
			</div>
...
//修改价格
	xgprise(){
		...
		if(this.mclass==1){
			this.detailInfo.goods_price1=this.detailInfo.goods_l_price
			this.priseName="会员价："
		}
		if(this.mclass==2){
			this.detailInfo.goods_price1=this.detailInfo.goods_c_price
			this.priseName="社区团购："
		}
		if(this.mclass==3){
			this.detailInfo.goods_price1=this.detailInfo.goods_p_price
			this.priseName="批发价："
		}
		if(this.mclass==4){
			this.detailInfo.goods_price1=this.detailInfo.goods_B_price
			this.priseName="B团会员："
		}
		if(this.mclass==5){
			this.detailInfo.goods_price1=this.detailInfo.goods_j_price
			this.priseName="集采价："
		}

		this.pricePer=this.detailInfo.goods_price1

		this.realprice=this.pricePer*this.detailInfo.goods_price
