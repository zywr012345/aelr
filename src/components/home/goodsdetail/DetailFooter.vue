	...
	>
		<div class="footer-flex">
			...
			<div class="right" v-if="mclass>0">
				...
				<div class="button disabled-cart"
             v-on:click="addShopping(true)"
             v-if="!detailInfo.bargain_info && ((!detailInfo.pintuan_type && detailInfo.goods_storage <= 0 && detailInfo.cart) || (detailInfo.pintuan_type && detailInfo.goods_storage <= 0 && detailInfo.pintuangroup_list.length))">
					{{detailInfo.pintuan_type?'参加拼团':'加入购物车'}}
				</div>
				<div
...
//修改价格
		let m_per=1
		if(this.mclass==1){m_per=this.detailInfo.goods_l_price}
		if(this.mclass==2){m_per=this.detailInfo.goods_c_price}
		if(this.mclass==3){m_per=this.detailInfo.goods_p_price}
		if(this.mclass==4){m_per=this.detailInfo.goods_B_price}
		if(this.mclass==5){m_per=this.detailInfo.goods_j_price}
