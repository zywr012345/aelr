...
<div class="content" slot="content">
  <baidu-map 
	...
	<bml-marker-clusterer :averageCenter="true">
    		<bm-marker v-for='(item,index) in items '  :position="{lng:item.lng,lat:item.lat}"  :title="item.name" @click="infoWindowOpen(item)"  :icon="{url:geticon(item.id), size: {width: 32, height: 32}}">
    		</bm-marker>
