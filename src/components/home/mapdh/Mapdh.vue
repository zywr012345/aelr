...
<div v-for='(item,index) in items ' :key="index">
    		<bm-marker  :position="{lng:item.lng,lat:item.lat}"  :title="item.name" @touchend="infoWindowOpen(index)"  @click="infoWindowOpen(index)"  :icon="{url:geticon(item.id), size: {width: 32, height: 32}}">
    		</bm-marker>
</div>
